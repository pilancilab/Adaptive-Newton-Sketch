...
np.random.seed(1)
A,b = sklearn.datasets.load_svmlight_file('{}/w7a/w7a'.format(data_dir))
A = A.A
indn = np.arange(24692)
...
with open('{}/w7a/w7a_kernel_train.p'.format(data_dir),'wb') as f:
    K_part = Ktrain
    b_part = btrain
    pickle.dump([K_part,b_part],f)

with open('{}/w7a/w7a_kernel_test.p'.format(data_dir),'wb') as f:
    K_part = Ktest
    b_part = btest
    pickle.dump([K_part,b_part],f)
